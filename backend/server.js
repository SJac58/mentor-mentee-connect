...
app.get("/api/dashboard/:mentorId", async (req, res) => {
  ...
  try {
    ...
    const avgProgress = totalMentees > 0 ? (totalProgress / totalMentees).toFixed(2) : 0;
    const avgAttendance = totalMentees > 0 ? (totalAttendance / totalMentees).toFixed(2) : 0;
...
app.get("/api/student/:regNum", async (req, res) => {
  ...
  try {
    const [students] = await db.query(
      `SELECT student_reg_num, name, class, dept, semester, year 
       FROM students WHERE student_reg_num = ?`,
      [regNum]
    );
    ...
    const meetingsPast = meetings.filter((m) => {
      const dateStr = new Date(m.meeting_date).toISOString().slice(0, 10);
...
app.post("/api/notes", async (req, res) => {
  ...
  if (!mentor_id || !student_reg_num || !note) {
    return res.status(400).json({ message: "mentor_id, student_reg_num and note are required" });
  }
  try {
...
app.post("/api/meetings", async (req, res) => {
  ...
  if (!student_reg_num || !meeting_date || !time_slot) {
    return res.status(400).json({ message: "All fields are required" });
  }
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO meetings 
       (mentor_id, student_reg_num, meeting_date, time_slot, reason, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
